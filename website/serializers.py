...
class EnderecoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Endereco
        fields = ['id','bairro', 'rua', 'numero_casa', 'complemento', 'cep', 'cidade', 'uf']
        read_only_fields = ['id']
...
class ClienteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cliente
        fields = ['user','id','nome', 'sobrenome', 'cpf', 'rg', 'data_nascimento', 'sexo', 'endereco']
        read_only_fields = ['id']
...
class VendaSerializer(serializers.ModelSerializer):
    vendas_produtos= VendaProdutoSerializer(source='vendas_produtos_venda', many=True)
    valor_total = serializers.DecimalField(max_digits=10, decimal_places=2, coerce_to_string=False, read_only=True)
    class Meta:
        model = Venda
    ...
    def criar_vendas_produtos(self, vendas_produtos_data, venda):
        for venda_produto_data in vendas_produtos_data:
            venda_produto = VendaProduto.objects.create(venda=venda, **venda_produto_data)
