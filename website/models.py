...
class Loja(ModelDate):
    ...
    razao_social = models.CharField('Razão Social', max_length=150, unique=True)
    avaliacoes = models.ManyToManyField(Cliente, related_name='avaliacoes', through='Avaliacao')
...
class Produto(ModelDate):
    ...
    descricao_completa = models.TextField('Descrição Completa', blank=True, null=True)
    logo = models.ImageField(
            upload_to='website/images', verbose_name='Imagem',
            null=True, blank=True)
...
class Venda(ModelDate):
    produtos = models.ManyToManyField(Produto, through='VendaProduto', related_name='vendas')
    cliente = models.ForeignKey(Cliente, on_delete=models.CASCADE, verbose_name='Cliente', related_name='vendas')
    loja = models.ForeignKey(Loja, on_delete=models.CASCADE, related_name='vendas')
    valor_total = models.DecimalField('Valor', max_digits=10, decimal_places=2, blank=True, default=Decimal('0.00'))

    def atualizar_valor(self):
        expression = Sum(F('valor') * F('quantidade'), output_field=models.DecimalField(max_digits=10,decimal_places=2))
        self.valor_total = self.vendas_produtos_venda.aggregate(valor_total=expression)['valor_total']
        self.save()                                                                
...
class VendaProduto(ModelDate):
    venda = models.ForeignKey(Venda, on_delete=models.CASCADE, related_name='vendas_produtos_venda')
    produto = models.ForeignKey(Produto, on_delete=models.CASCADE, related_name='vendas_produtos_produto')
    valor = models.DecimalField('Valor', max_digits=10, decimal_places=2)
...
class Avaliacao(ModelDate):
    cliente = models.ForeignKey(Cliente, on_delete=models.CASCADE, related_name='avaliacoes_cliente')
    loja = models.ForeignKey(Loja, on_delete=models.CASCADE, related_name='avaliacoes_loja')
